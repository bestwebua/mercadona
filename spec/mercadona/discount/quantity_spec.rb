...
RSpec.describe Mercadona::Discount::Quantity do
  describe '.call' do
    subject(:amount) { described_class.call(discount_rule, order_item, order_item_quantity) }

    let(:discount_rule) { create(:discount_rule, :quantity_discount) }
    ...
    context 'when discount case' do
      ...
      let(:expected_result) { amount_without_discount - ((discount_rule.discount * 100).to_i * order_item_quantity) }
